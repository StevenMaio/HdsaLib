...
public:
  MD_Opt_Prob_Interface_synthetic_test_OUU(int ens_size, HDSA::Ptr<HDSA::Dense_Matrix<RealT>> &Xi) : HDSA::MD_OUU_Opt_Prob_Interface<RealT>(ens_size), Xi_(Xi)
  {
    m_ = 51;
  ...
  void Apply_RS_Hessian_Per_Sample(HDSA::Vector<RealT> &z_out, const HDSA::Vector<RealT> &z_in, const HDSA::Vector<RealT> &z, int s) const
  {
    ...
    for (int k = 0; k < m_; k++)
    {
      v->Set_Entry(k, 0, (*Xi_)(0, s) * 9.0 * (z_in_std(k) * std::pow(z_std(k), 2.0)));
    }
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> M_v = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
    M_->Multiply(*M_v, *v);
    for (int k = 0; k < m_; k++)
    {
      z_out_std.Set_Entry(k, (*Xi_)(0, s) * (*M_v)(k, 0) * std::pow(z_std(k), 2.0));
    }
  }
