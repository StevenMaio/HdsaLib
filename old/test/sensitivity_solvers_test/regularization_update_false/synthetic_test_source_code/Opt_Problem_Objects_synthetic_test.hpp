...
public:
  ...
  { 
    ...
    HDSA::Opt_Problem_Objects<RealT>::fs_obj = HDSA::makePtr<synthetic_test_FS_Objective<RealT> >(a);
    HDSA::Opt_Problem_Objects<RealT>::con = HDSA::makePtr<synthetic_test_Constraint<RealT> >();
    HDSA::Opt_Problem_Objects<RealT>::u = HDSA::makePtr<Std_Vector<RealT> >(z_dim);
    HDSA::Opt_Problem_Objects<RealT>::lambda = HDSA::makePtr<Std_Vector<RealT> >(z_dim);
  ...
  {
    int a_dim = a_.size();
    HDSA::Opt_Problem_Objects<RealT>::u->zero();
    HDSA::Opt_Problem_Objects<RealT>::z->zero();
    HDSA::Opt_Problem_Objects<RealT>::lambda->zero();
    for(int k = 0; k < a_dim; k++)
      {
	HDSA::Opt_Problem_Objects<RealT>::u->Replace_Element(k,a_[k]);
