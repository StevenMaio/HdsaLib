...
template <class Real>
class MeshManager_CDR : public MeshManager_Rectangle<Real> {
...
public: 
  ...
  MeshManager_CDR(Teuchos::ParameterList &parlist) : MeshManager_Rectangle<Real>(parlist)
  {
  ...
  ROL::Ptr<std::vector<std::vector<std::vector<int> > > > getSideSets(
    ...
    if (verbose) {
      outStream << "Mesh_CDR: getSideSets called" << std::endl;
      outStream << "Mesh_CDR: numSideSets = "     << meshSideSets_->size() << std::endl;
    }
