#ifndef PDE_SHALLOW_ICE_BEDROCK_HPP
#define PDE_SHALLOW_ICE_BEDROCK_HPP
...
template <class Real>
class PDE_shallow_ice_bedrock : public PDE<Real> {
private:
...
public:
  PDE_shallow_ice_bedrock(Teuchos::ParameterList &parlist) {
   ...
   void residual(ROL::Ptr<Intrepid::FieldContainer<Real> > & res,
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > valHflux_eval =
      ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    compute_parameters(valB_eval,valHflux_eval);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > kappa
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeBasalVelocity(kappa,valBeta_eval,valS_eval,0);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > h
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeThermalVelocity(h,valS_eval,A,0);
   ...
   // MULTIPLY h * grad(S) . grad(S) * grad(S)
    ...
    computeSurfaceVelocity(v,valS_eval,A_,0);
  ...
  void Jacobian_1(ROL::Ptr<Intrepid::FieldContainer<Real> > & jac,
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > d_kappa
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeBasalVelocity(d_kappa,valBeta_eval,valS_eval,1);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > d_h
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeThermalVelocity(d_h,valS_eval,A,1);
  ...
  void Jacobian_2(ROL::Ptr<Intrepid::FieldContainer<Real> > & jac,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Jacobian_2): Jacobian is zero.");
  }
  ...
  void Jacobian_3(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & jac,
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > kappa
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeBasalVelocity(kappa,valBeta_eval,valS_eval,1);

    // COMPUTE THERMAL VELOCITY
    Real A = A_;
    ROL::Ptr<Intrepid::FieldContainer<Real> > h
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeThermalVelocity(h,valS_eval,A,1);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > I = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    Intrepid::RealSpaceTools<Real>::add(*I, *kappa);
    Intrepid::RealSpaceTools<Real>::add(*I, h_gradS_gradS);
    Intrepid::RealSpaceTools<Real>::scale(*I,-1.0);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > Vx = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    ROL::Ptr<Intrepid::FieldContainer<Real> > Vy = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    for(int i = 0; i < c; i++)
      {
        for(int j = 0; j < p; j++)
          {
            (*Vx)(i,j) = -(*gradS_eval)(i,j,0)*dv1_gradS_gradS(i,j);
            (*Vy)(i,j) = -(*gradS_eval)(i,j,1)*dv1_gradS_gradS(i,j);
          }
      }
	...
	ROL::Ptr<Intrepid::FieldContainer<Real> > valB_i_eval
	  = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
	compute_bedrock_theta_deriv(valB_i_eval,i);

	ROL::Ptr<Intrepid::FieldContainer<Real> > Ii = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
	Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(*Ii,
								   *I,
								   *valB_i_eval);
	ROL::Ptr<Intrepid::FieldContainer<Real> > Ii_gradS = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p, d);
	Intrepid::FunctionSpaceTools::tensorMultiplyDataData<Real>(*Ii_gradS,
								   *Ii,
								   *gradS_eval);
	...
	ROL::Ptr<Intrepid::FieldContainer<Real> > Vxi = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
	Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(*Vxi,
								   *Vx,
								   *valB_i_eval);

	ROL::Ptr<Intrepid::FieldContainer<Real> > Vyi = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
	Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(*Vyi,
								   *Vy,
								   *valB_i_eval);

	Intrepid::FunctionSpaceTools::integrate<Real>(*J[1],
						      *Vxi,
						      *(fe_->NdetJ()),
						      Intrepid::COMP_CPP, false);

	Intrepid::FunctionSpaceTools::integrate<Real>(*J[2],
						      *Vyi,
						      *(fe_->NdetJ()),
						      Intrepid::COMP_CPP, false);
  ...
  void Hessian_11(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_11): Hessian is zero.");
  }
  ...
  void Hessian_12(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_12): Hessian is zero.");
  }
  ...
  void Hessian_13(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_13): Hessian is zero.");
  }
  ...
  void Hessian_21(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_21): Hessian is zero.");
  }
  ...
  void Hessian_22(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_22): Hessian is zero.");
  }
  ...
  void Hessian_23(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_23): Hessian is zero.");
  }
  ...
  void Hessian_31(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_31): Hessian is zero.");
  }
  ...
  void Hessian_32(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_32): Hessian is zero.");
  }
  ...
  void Hessian_33(std::vector<std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > > & hess,
                  ...
                  const ROL::Ptr<const Intrepid::FieldContainer<Real> > & z_coeff = ROL::nullPtr,
                  const ROL::Ptr<const std::vector<Real> > & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_33): Hessian is zero.");
  }

   void RieszMap_1(ROL::Ptr<Intrepid::FieldContainer<Real> > & riesz) {
     throw Exception::NotImplemented(">>> (PDE_shallow_ice_bedrock::RieszMap_1): Not implemented.");
  }

  void RieszMap_2(ROL::Ptr<Intrepid::FieldContainer<Real> > & riesz) {
    throw Exception::NotImplemented(">>> (PDE_shallow_ice_bedrock::RieszMap_2): Not implemented.");
  }
...
private:
  ...
  void computeBasalVelocity(ROL::Ptr<Intrepid::FieldContainer<Real> > &kappa,
	...
	pt[1] = (*fe_->cubPts())(i,j,1)/height_;
        // Compute basal velocity
        (*kappa)(i,j) = evaluateBasalVelocity((*beta)(i,j),(*u)(i,j),pt,deriv);
      }
    }
  }

  Real evaluateBasalVelocity(const Real beta, const Real u, const std::vector<Real> & x, const int deriv = 0) const {
    Real val = 0.0;
    if(deriv == 0)
      {
	val = beta*rho_*g_*std::pow(u-bedrock(x),2.0);
      }
    if(deriv == 1)
      {
	val = beta*rho_*g_*2.0*(u-bedrock(x));
      }
    if(deriv == 2)
      {
  ...
  void computeThermalVelocity(ROL::Ptr<Intrepid::FieldContainer<Real> > &h,
	...
	pt[1] = (*fe_->cubPts())(i,j,1)/height_;
        // Compute thermal velocity
        (*h)(i,j) = evaluateThermalVelocity((*u)(i,j),pt,A,deriv);
      }
    }
  }

  Real evaluateThermalVelocity(const Real u, const std::vector<Real> & x, const Real A, const int deriv = 0) const {
    Real val = 0.0;
    if(deriv == 0)
      {
	val = 0.4*A*std::pow(rho_*g_,3.0)*std::pow(u-bedrock(x),5.0);
      }
    if(deriv == 1)
      {
	val = 0.4*A*std::pow(rho_*g_,3.0)*5.0*std::pow(u-bedrock(x),4.0);
      }
    if(deriv == 2)
      {
        val = 0.4*A*std::pow(rho_*g_,3.0)*20.0*std::pow(u-bedrock(x),3.0);
      }
    return val;
  }
  ...
  void computeSurfaceVelocity(ROL::Ptr<Intrepid::FieldContainer<Real> > &v,
	...
	pt[1] = (*fe_->cubPts())(i,j,1)/height_;
        // Compute thermal velocity
        (*v)(i,j) = evaluateSurfaceVelocity((*u)(i,j),pt,A,deriv);
      }
    }
  }

  Real evaluateSurfaceVelocity(const Real u, const std::vector<Real> & x, const Real A, const int deriv = 0) const {
    Real val = 0.0;
    if(deriv == 0)
      {
	val = 0.5*A*std::pow(rho_*g_,3.0)*std::pow(u-bedrock(x),4.0);
      }
    if(deriv == 1)
      {
	val = 0.5*A*std::pow(rho_*g_,3.0)*4.0*std::pow(u-bedrock(x),3.0);
      }
    if(deriv == 2)
      {
        val = 0.5*A*std::pow(rho_*g_,3.0)*12.0*std::pow(u-bedrock(x),2.0);
      }
    return val;
  }

  void compute_parameters(ROL::Ptr<Intrepid::FieldContainer<Real> > & b, ROL::Ptr<Intrepid::FieldContainer<Real> > & h_flux) const {
    int c = fe_->gradN()->dimension(0);
    int p = fe_->gradN()->dimension(2);
    int d = fe_->gradN()->dimension(3);
    std::vector<Real> pt(d);
  ...
  void compute_bedrock_theta_deriv(ROL::Ptr<Intrepid::FieldContainer<Real> > & b, int partial) const {
    int c = fe_->gradN()->dimension(0);
    int p = fe_->gradN()->dimension(2);
    int d = fe_->gradN()->dimension(3);
    std::vector<Real> pt(d);
	...
	pt[0] = (*fe_->cubPts())(i,j,0)/width_;
	pt[1] = (*fe_->cubPts())(i,j,1)/height_;
        (*b)(i,j) = bedrock_theta_deriv(pt,partial);
      }
    }
	...
	for(int jj = -1; jj < 2; jj++)
	  {
		...
		val += bedrock_topo_coeff_[k]*Linear_FE_Basis_Fun_Eval(x,i_index)*Linear_FE_Basis_Fun_Eval(y,j_index);
	      }
	  }
      }
  ...
  Real bedrock_theta_deriv(const std::vector<Real> & pt, int partial) const {
    Real val = 0.0;
    Real x = pt[0];
    Real y = pt[1];
    ...
    std::vector<Real> param = std::vector<Real>((L_+1)*(L_+1),0.0);
    param[partial] = 1.0;
    if((int)param.size()>0)
      {
	Real val_uncertain = 0.0;
  ...
  Real rhs(const std::vector<Real> & pt) const {
    ...
    Real pi(M_PI);
    Real time_coeff = 1.0 + 0.3*std::sin(2.0*pi*t);
