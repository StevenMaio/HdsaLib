...
public:
  ...
  void Jacobian_2(ROL::Ptr<Intrepid::FieldContainer<Real> > & jac,
    ...
    Intrepid::FunctionSpaceTools::tensorMultiplyDataData<Real>(kappa_gradS,
                                                               *kappa,
                                                               *gradS_eval);
    // MULTIPLY (kappa * grad(S)) . grad(N)
    Intrepid::FieldContainer<Real> kappa_gradS_gradN(c, f, p);
    Intrepid::FunctionSpaceTools::dotMultiplyDataField<Real>(kappa_gradS_gradN,
                                                             kappa_gradS,
                                                             *(fe_->gradNdetJ()));
    // INTEGRATE (kappa * grad(S)) . grad(N) * N
    Intrepid::FunctionSpaceTools::integrate<Real>(*J[0][0],
                                                  kappa_gradS_gradN,
                                                  *(fe_->N()),
                                                  Intrepid::COMP_CPP, true);
  ...
  void Hessian_11(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > d1_kappa
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeBasalVelocity(d1_kappa,valBeta_eval,valS_eval,1);
    ROL::Ptr<Intrepid::FieldContainer<Real> > d2_kappa
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeBasalVelocity(d2_kappa,valBeta_eval,valS_eval,2);
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > d1_h
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeThermalVelocity(d1_h,valS_eval,A,1);
    ROL::Ptr<Intrepid::FieldContainer<Real> > d2_h
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeThermalVelocity(d2_h,valS_eval,A,2);
   ...
   // MULTIPLY 2.0 * h * grad(S) . grad(L) * grad(N)
    ...
    ROL::Ptr<Intrepid::FieldContainer<Real> > d2_v
      = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, p);
    computeSurfaceVelocity(d2_v,valS_eval,A,2);
  ...
  void Hessian_12(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
    ...
    Intrepid::FunctionSpaceTools::dotMultiplyDataData<Real>(gradS_gradL,
                                                            *gradS_eval,
                                                            *gradL_eval);
    // MULTIPLY d1_kappa * grad(S) . grad(L)
    Intrepid::FieldContainer<Real> d1_kappa_gradS_gradL(c, p);
    Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(d1_kappa_gradS_gradL,
                                                               *d1_kappa,
                                                               gradS_gradL);
    // MULTIPLY d1_kappa * grad(S) . grad(L) * N
    Intrepid::FieldContainer<Real> d1_kappa_gradS_gradL_N(c, f, p);
    Intrepid::FunctionSpaceTools::scalarMultiplyDataField<Real>(d1_kappa_gradS_gradL_N,
                                                                d1_kappa_gradS_gradL,
                                                                *(fe_->N()));
    // INTEGRATE (d1_kappa * grad(S) . grad(L) ) * N * N
    Intrepid::FunctionSpaceTools::integrate<Real>(*H[0][0],
                                                  d1_kappa_gradS_gradL_N,
                                                  *(fe_->NdetJ()),
                                                  Intrepid::COMP_CPP, false);

    // MULTIPLY kappa * grad(L)
    Intrepid::FieldContainer<Real> kappa_gradL(c, p, d);
    Intrepid::FunctionSpaceTools::tensorMultiplyDataData<Real>(kappa_gradL,
                                                               *kappa,
                                                               *gradL_eval);
    // MULTIPLY (kappa * grad(L)) . grad(N)det(J)
    Intrepid::FieldContainer<Real> kappa_gradL_gradNdetJ(c, f, p);
    Intrepid::FunctionSpaceTools::dotMultiplyDataField<Real>(kappa_gradL_gradNdetJ,
                                                             kappa_gradL,
                                                             *(fe_->gradNdetJ()));
    // INTEGRATE (kappa * grad(L)) . grad(N) * N
    Intrepid::FunctionSpaceTools::integrate<Real>(*H[0][0],
                                                  *(fe_->N()),
                                                  kappa_gradL_gradNdetJ,
                                                  Intrepid::COMP_CPP, true);
    // Combine the hessians
    fieldHelper_->combineFieldCoeff(hess,H);
  ...
  void Hessian_21(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
   ...
   // INITILAIZE HESSIAN
    ...
    Intrepid::FunctionSpaceTools::tensorMultiplyDataData<Real>(kappa_gradL,
                                                               *kappa,
                                                               *gradL_eval);
    // MULTIPLY kappa * grad(L) . grad(N)
    Intrepid::FieldContainer<Real> kappa_gradL_gradN(c, f, p);
    Intrepid::FunctionSpaceTools::dotMultiplyDataField<Real>(kappa_gradL_gradN,
                                                             kappa_gradL,
                                                             *(fe_->gradN()));
    // INTEGRATE (kappa * grad(L)) . grad(N) * N
    Intrepid::FunctionSpaceTools::integrate<Real>(*H[0][0],
  ...
  void Hessian_22(ROL::Ptr<Intrepid::FieldContainer<Real> > & hess,
   ...
   // INITILAIZE HESSIAN
    ...
    Intrepid::FunctionSpaceTools::dotMultiplyDataData<Real>(gradS_gradL,
                                                            *gradS_eval,
                                                            *gradL_eval);
    // MULTIPLY kappa * grad(S) . grad(L)
    Intrepid::FieldContainer<Real> kappa_gradS_gradL(c, p);
    Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(kappa_gradS_gradL,
                                                               *kappa,
                                                               gradS_gradL);
    // MULTIPLY kappa * grad(S) . grad(L) * N
    Intrepid::FieldContainer<Real> kappa_gradS_gradL_N(c, f, p);
    Intrepid::FunctionSpaceTools::scalarMultiplyDataField<Real>(kappa_gradS_gradL_N,
                                                                kappa_gradS_gradL,
                                                                *(fe_->N()));
    // INTEGRATE (kappa * grad(S) . grad(L) ) * N * N
    Intrepid::FunctionSpaceTools::integrate<Real>(*H[0][0],
                                                  kappa_gradS_gradL_N,
                                                  *(fe_->NdetJ()),
                                                  Intrepid::COMP_CPP, false);
