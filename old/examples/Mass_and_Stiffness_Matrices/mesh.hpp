...
template <class Real>
class MeshManager_Construction : public MeshManager_Rectangle<Real> {
...
public: 
  ...
  MeshManager_Construction(Teuchos::ParameterList &parlist) : MeshManager_Rectangle<Real>(parlist)
  {
  ...
  ROL::Ptr<std::vector<std::vector<std::vector<int> > > > getSideSets(
    ...
    if (verbose) {
      outStream << "Mesh_Construction: getSideSets called" << std::endl;
      outStream << "Mesh_Construction: numSideSets = "     << meshSideSets_->size() << std::endl;
    }
    return meshSideSets_;
  }
  
}; // MeshManager_Construction
