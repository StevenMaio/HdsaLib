#ifndef REDUCED_OBJECTIVE_SIMOPT_MOD_H
#define REDUCED_OBJECTIVE_SIMOPT_MOD_H
...
template <class Real>
class Reduced_Objective_SimOpt_modified : public ROL::Objective<Real> {
private:
...
public:
  ...
  */
  Reduced_Objective_SimOpt_modified(
  ...
  */
  ...
  void set_state_solution(const ROL::Vector<Real> & u, const ROL::Vector<Real> &z, Real &tol) { 
    // Update equality constraint with new Opt variable.
    con_->update_2(z,updateFlag_,updateIter_);
    // Solve state equation.
    state_->set(u);
...
}; // class Reduced_Objective_SimOpt_modified
