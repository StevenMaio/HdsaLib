#ifndef HDSA_BELOS_INTERFACE_HPP
#define HDSA_BELOS_INTERFACE_HPP
...
public:
  ...
  HDSA_Belos_Vector(const HDSA::Ptr<HDSA::Vector<RealT> > & vec_in, const int & NumVecs): 
    NumVecs_(NumVecs)
  {
    vec.resize(NumVecs_);
  ...
  void MvTransMv (RealT alpha, const Belos::MultiVec<RealT>& A, 
                  Teuchos::SerialDenseMatrix< int, RealT >& B
                 ) const
  {
...
public:
  ...
  HDSA_Belos_Operator(const HDSA::Ptr<HDSA::Linear_Operator<RealT> > & A): A_(A)
  { }
