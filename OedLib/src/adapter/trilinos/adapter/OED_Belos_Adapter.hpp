#ifndef OED_BELOS_ADAPTER_HPP
#define OED_BELOS_ADAPTER_HPP
...
namespace OED
{
  ...
  namespace Trilinos_Adapter
  {
    ...
    public:
      std::vector<OED::Ptr<OED::Vector<RealT>>> vec;

      Belos_Vector(const OED::Vector<RealT> &vec_in, const int &NumVecs) : NumVecs_(NumVecs)
      {
        vec.resize(NumVecs_);
    ...
    {
      const OED::Linear_Operator<RealT> *A_;

    public:
      Belos_Operator(const OED::Linear_Operator<RealT> *A) : A_(A)
      {
      }
