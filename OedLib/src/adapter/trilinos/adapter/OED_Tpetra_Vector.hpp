#ifndef OED_TPETRA_VECTOR_HPP
#define OED_TPETRA_VECTOR_HPP
...
namespace OED
{
  ...
  namespace Trilinos_Adapter
  {
    ...
    class Tpetra_Vector : public OED::Vector<RealT>
    {
    ...
    public:
      ...
      OED::Ptr<Tpetra::MultiVector<RealT, LO, GO, Node>> getVector() const
      {
        return tpetra_vec_;
      }

      OED::Ptr<OED::Vector<RealT>> Clone() const override
      {
        int n = tpetra_vec_->getNumVectors();
        return OED::makePtr<Tpetra_Vector>(OED::makePtr<Tpetra::MultiVector<RealT, LO, GO, Node>>(map_, n), random_number_generator_);
      }
