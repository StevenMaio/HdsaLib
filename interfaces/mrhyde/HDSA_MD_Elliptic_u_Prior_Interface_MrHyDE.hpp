...
private:
    ...
    std::vector<Teuchos::RCP<Tpetra::CrsMatrix<ScalarT,LO,GO,Node> >> E_u_;
  std::vector<HDSA::Ptr<HDSA::Sparse_Matrix_Solver<RealT> > > E_u_solver_;
  
public:
  MD_Elliptic_u_Prior_Interface_MrHyDE(RealT & alpha_u, RealT & beta_u, HDSA::Ptr<Prior_FE_Op_MrHyDE<RealT>> &prior_fe_op, HDSA::Ptr<HDSA::Vector<RealT> > &uvec, int &prior_num_sing_vals, int &prior_oversampling, int &prior_num_subspace_iter):
  ...
  MD_Elliptic_u_Prior_Interface_MrHyDE(RealT & alpha_u, RealT & beta_u, HDSA::Ptr<Prior_FE_Op_MrHyDE<RealT>> &prior_fe_op, HDSA::Ptr<HDSA::Vector<RealT> > &uvec, int &prior_num_sing_vals, int &prior_oversampling, int &prior_num_subspace_iter, int & seed):
    HDSA::MD_Elliptic_u_Prior_Interface<RealT>(alpha_u,seed),prior_fe_op_(prior_fe_op)
  {
    Construct_Elliptic_Operator(beta_u,uvec,prior_num_sing_vals,prior_oversampling,prior_num_subspace_iter);
  }

  MD_Elliptic_u_Prior_Interface_MrHyDE(RealT & alpha_u, RealT & beta_u, HDSA::Ptr<Prior_FE_Op_MrHyDE<RealT>> &prior_fe_op, HDSA::Ptr<HDSA::Vector<RealT> > &uvec, int &prior_num_sing_vals, int &prior_oversampling,int &prior_num_subspace_iter, const HDSA::Ptr<HDSA::Random_Number_Generator<RealT> > & random_number_generator):
  ...
  void Construct_Elliptic_Operator(RealT &beta_u, HDSA::Ptr<HDSA::Vector<RealT> > &uvec, int &prior_num_sing_vals, int &prior_oversampling,int &prior_num_subspace_iter) {
	...
	E_u_solver_[i] = HDSA::makePtr<HDSA::Sparse_Matrix_Solver<RealT>>(E_u_[i]);
      }
