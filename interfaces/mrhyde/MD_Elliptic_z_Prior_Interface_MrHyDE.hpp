...
public:
  ...
  void Apply_E_z_Inverse(HDSA::Vector<RealT> & z_out, const HDSA::Vector<RealT> & z_in) const {
    const Vector_MrHyDE<RealT> &ez_in = dynamic_cast<const Vector_MrHyDE<RealT>&>(z_in);  
    Vector_MrHyDE<RealT> &ez_out = dynamic_cast<Vector_MrHyDE<RealT>&>(z_out);
    ...
    HDSA::Ptr<HDSA::Vector<RealT> > vec_in = HDSA::makePtr<Tpetra_Vector<RealT> > (eez_in.getField()[0]->getVector(),ez_in.Get_random_number_generator());
    HDSA::Ptr<HDSA::Vector<RealT> > vec_out = HDSA::makePtr<Tpetra_Vector<RealT> > (eez_out.getField()[0]->getVector(),ez_out.Get_random_number_generator()); 
