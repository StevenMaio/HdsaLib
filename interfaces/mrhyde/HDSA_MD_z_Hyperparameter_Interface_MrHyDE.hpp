...
private:
  HDSA::Ptr<MD_Data_Interface_MrHyDE<RealT>> data_interface_;
...
public:
  std::vector<std::vector<RealT>> Spatial_Domain_Bounds(void) const override
  {
    ...
    Teuchos::RCP<Teuchos::MpiComm<int>> comm = data_interface_->Get_Communicator();
    ...
    for (int i = 0; i < dim; i++)
    {
      ...
      Teuchos::reduceAll(*comm, Teuchos::REDUCE_MIN, 1, &minVal, &globalMin);
      Teuchos::reduceAll(*comm, Teuchos::REDUCE_MAX, 1, &maxVal, &globalMax);
  ...
  {
    data_interface_->State_Solve(u, z);
  }

  MD_z_Hyperparameter_Interface_MrHyDE(const HDSA::Ptr<MD_Data_Interface_MrHyDE<RealT>> data_interface, const std::string &z_type, const int &num_state_solves = 0) : HDSA::MD_z_Hyperparameter_Interface<RealT>(z_type, num_state_solves)
  {
    data_interface_ = data_interface;
  }

  MD_z_Hyperparameter_Interface_MrHyDE(const HDSA::Ptr<MD_Data_Interface_MrHyDE<RealT>> data_interface, int seed, const std::string &z_type, const int &num_state_solves = 0) : HDSA::MD_z_Hyperparameter_Interface<RealT>(seed, z_type, num_state_solves)
  {
    data_interface_ = data_interface;
  }

  MD_z_Hyperparameter_Interface_MrHyDE(const HDSA::Ptr<MD_Data_Interface_MrHyDE<RealT>> data_interface, const HDSA::Ptr<HDSA::Random_Number_Generator<RealT>> &random_number_generator, const std::string &z_type, const int &num_state_solves = 0) : HDSA::MD_z_Hyperparameter_Interface<RealT>(random_number_generator, z_type, num_state_solves)
  {
    data_interface_ = data_interface;
  }
};
