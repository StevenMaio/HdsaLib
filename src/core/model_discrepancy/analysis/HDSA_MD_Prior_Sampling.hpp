...
namespace HDSA
{
  ...
  public:
    ...
    {
      HDSA::Ptr<HDSA::MultiVector<RealT> > delta_samples = HDSA::makePtr<HDSA::MultiVector<RealT> >(num_samples,*data_interface_->u_opt_);

      for(int i = 0; i < num_samples; i++)
	{
	  // Apply W_u^{-1} to random vector                   
	  HDSA::Ptr<HDSA::Vector<RealT> > u_in = data_interface_->u_opt_;
    ...
    std::vector<HDSA::Ptr<HDSA::MultiVector<RealT> > > Prior_Discrepancy_Samples(const HDSA::MultiVector<RealT> & z, const int & num_samples)
    {
      ...
      HDSA::Ptr<HDSA::MultiVector<RealT> > Z = HDSA::makePtr<HDSA::MultiVector<RealT> >(N,*data_interface_->z_opt_);
      HDSA::Ptr<HDSA::MultiVector<RealT> > W_z_inv_Z = HDSA::makePtr<HDSA::MultiVector<RealT> >(N,*data_interface_->z_opt_);
      for(int k = 0; k < N; k++)
	{
	  HDSA::Ptr<HDSA::Vector<RealT> > z_tmp1 = (*Z)[k];
	  z_tmp1->set(*z[k]);
	  z_tmp1->axpy(-1.0,*data_interface_->z_opt_);
      ...
      for(int i = 0; i < num_samples; i++)
	{
	 delta_samples[i] = HDSA::makePtr<HDSA::MultiVector<RealT> >(N,*data_interface_->u_opt_);

	 // Generate random sample i 
	 HDSA::Ptr<HDSA::Vector<RealT> > u_intercept = data_interface_->u_opt_->clone();
	 u_intercept->randomize_standard_normal();
	 HDSA::Ptr<HDSA::MultiVector<RealT> > u_rand = HDSA::makePtr<HDSA::MultiVector<RealT> >(N,*data_interface_->u_opt_);
	 u_rand->randomize_standard_normal();

	 for(int k = 0; k < N; k++)
	   {
	     // Form random sample for input z[k]
	     HDSA::Ptr<HDSA::Vector<RealT> > u_in = data_interface_->u_opt_->clone();
