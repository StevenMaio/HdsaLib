...
namespace HDSA
{
  ...
  public:
    ...
    { 
      state_grad_ = data_interface_->u_opt->clone();
      opt_prob_interface_->Misfit_Gradient(*state_grad_,*data_interface_->u_opt,*data_interface_->z_opt);
    ...
    {
      HDSA::Ptr<HDSA::MD_Posterior_Vectors<RealT> > posterior_samples = HDSA::makePtr<HDSA::MD_Posterior_Vectors<RealT> >(post_sampling_->post_data->num_samples,*data_interface_->z_opt);
      HDSA::Ptr<HDSA::Vector<RealT> > z_update_mean = Posterior_Update_Mean();
      posterior_samples->mean->set(*z_update_mean);

      //////////////////// B_theta_hat
      HDSA::Ptr<HDSA::MultiVector<RealT> > u_tmp1 =  HDSA::makePtr<HDSA::MultiVector<RealT> >(post_sampling_->post_data->num_samples,*data_interface_->u_opt);
      HDSA::Ptr<HDSA::MultiVector<RealT> > B_theta_hat =  HDSA::makePtr<HDSA::MultiVector<RealT> >(post_sampling_->post_data->num_samples,*data_interface_->z_opt);
      for(int i = 0; i < post_sampling_->post_data->N; i++)
	{
	  ...
	  // Compute W_z_inv_yi                                                                                                                                                                                                               
	  HDSA::Ptr<HDSA::Vector<RealT> > W_z_inv_yi = data_interface_->z_opt->clone();
      ...
      for(int k = 0; k < post_sampling_->post_data->num_samples; k++)
	{
	  HDSA::Ptr<HDSA::Vector<RealT> > u_tmp2 = data_interface_->u_opt->clone();
	  opt_prob_interface_->Apply_Misfit_Hessian(*u_tmp2,*(*u_tmp1)[k],*data_interface_->u_opt,*data_interface_->z_opt);
	  
	  HDSA::Ptr<HDSA::Vector<RealT> > z_tmp1 = data_interface_->z_opt->clone();
	  opt_prob_interface_->Apply_Solution_Operator_z_Jacobian_Transpose(*z_tmp1,*u_tmp2,*data_interface_->z_opt);
      ...
      HDSA::Ptr<HDSA::MultiVector<RealT> > B_theta_breve =  HDSA::makePtr<HDSA::MultiVector<RealT> >(post_sampling_->post_data->num_samples,*data_interface_->z_opt);
      for(int k = 0; k < post_sampling_->post_data->num_samples; k++)
	{
      ...
      for(int k = 0; k < post_sampling_->post_data->num_samples; k++)
	{
	  HDSA::Ptr<HDSA::Vector<RealT> > z_tmp2 = data_interface_->z_opt->clone();
	  z_tmp2->set(*(*B_theta_hat)[k]);
	  z_tmp2->plus(*(*B_theta_breve)[k]);
	  hessian_analysis_->Apply_RS_Hessian_Inverse(*(*z_update_samples)[k],*z_tmp2,*data_interface_->z_opt);
    ...
    {
      HDSA::Ptr<HDSA::Vector<RealT> > z_update_mean = data_interface_->z_opt->clone();
      ...
      for(int ell = 0; ell < post_sampling_->post_data->N; ell++)
	{
      ...
      opt_prob_interface_->Apply_Misfit_Hessian(*u_tmp2,*u_tmp1,*data_interface_->u_opt,*data_interface_->z_opt);
      ...
      for(int ell = 0; ell < post_sampling_->post_data->N; ell++)
	{
      ...
      hessian_analysis_->Apply_RS_Hessian_Inverse(*z_update_mean,*z_tmp1,*data_interface_->z_opt);
      z_update_mean->plus(*data_interface_->z_opt);
