...
int main(int argc, char *argv[]) {
  ...
  std::vector<RealT> b = std::vector<RealT>(a_dim,0.0);
  for(int k = 0; k < a_dim; k++)
    {
      b[k] = 1.0/static_cast<RealT>(k+1);
    }

  int theta_dim = 20;  
  int z_dim = 25;

  HDSA::Ptr<HDSA::Opt_Problem_Objects<RealT> > OP_Objects_Factory = HDSA::makePtr<Opt_Problem_Objects_synthetic_test<RealT> >(a,b,z_dim);
