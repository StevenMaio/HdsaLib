...
private:
  ...
  HDSA::Ptr<ROL::Constraint_SimOpt<RealT> > con_hifi;
...
public:
    ...
    Opt_Problem_Objects_ROL<RealT>(parlist_in,comm), parlist(parlist_in)
  {
    ...
    RealT diff_coeff = parlist->sublist("Problem").get("Diffusion Coefficient", 1.0);
    RealT adv_coeff = 0.0;
    RealT cubic_react_coeff = 0.0;
    RealT linear_react_coeff = 0.0;
    HDSA::Ptr<PDE<RealT> > pde = HDSA::makePtr<PDE_CDR<RealT> >(*parlist,diff_coeff,adv_coeff,cubic_react_coeff,linear_react_coeff);
    ...
    RealT hifi_adv_coeff = parlist->sublist("Problem").get("Advection Coefficient", 1.0);
    HDSA::Ptr<PDE<RealT> > pde_hifi = HDSA::makePtr<PDE_CDR<RealT> >(*parlist,diff_coeff,hifi_adv_coeff,cubic_react_coeff,linear_react_coeff);
    con_hifi = HDSA::makePtr<PDE_Constraint<RealT> >(pde_hifi,meshMgr,comm->Get_Teuchos_Communicator(),*parlist,*outStream);
    ...
    if ( checkDeriv ) {
      ...
      HDSA::Ptr<ROL::Vector<RealT> > dzp = HDSA::makePtr<PDE_PrimalOptVector<RealT> >(dz_ptr,pde,assembler);
