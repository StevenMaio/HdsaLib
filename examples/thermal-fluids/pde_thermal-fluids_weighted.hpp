#ifndef PDE_THERMALFLUIDS_WEIGHTED_HPP
#define PDE_THERMALFLUIDS_WEIGHTED_HPP
...
template <class Real>
class PDE_ThermalFluids_Weighted : public PDE<Real> {
private:
...
public:
  PDE_ThermalFluids_Weighted(Teuchos::ParameterList &parlist, std::vector<Real> & weights) : grav_(-1), weights_(weights) {
  ...
  void residual(ROL::Ptr<Intrepid::FieldContainer<Real> > & res,
    ...
    for(int i = 0; i < 4; i++)
      {
	Intrepid::RealSpaceTools<Real>::scale(*R[i],weights_[i]);
      }
  ...
  void Jacobian_1(ROL::Ptr<Intrepid::FieldContainer<Real> > & jac,
	...
	for(int k = 0; k < 4; k++)
	  {
	    Intrepid::RealSpaceTools<Real>::scale(*J[i][k],weights_[i]);
	  }
      }    
...
//    throw Exception::NotImplemented("");
	...
	for(int k = 0; k < 4; k++)
	  {
	    Intrepid::RealSpaceTools<Real>::scale(*H[i][k],weights_[i]);
	  }
      }  
