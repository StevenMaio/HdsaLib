...
template <class Real>
class MeshManager_darcy_adv_diff : public MeshManager_Rectangle<Real> {
...
public: 
  ...
  MeshManager_darcy_adv_diff(Teuchos::ParameterList &parlist) : MeshManager_Rectangle<Real>(parlist)
  {
  ...
  ROL::Ptr<std::vector<std::vector<std::vector<int> > > > getSideSets(
    ...
    if (verbose) {
      outStream << "Mesh_darcy_adv_diff: getSideSets called" << std::endl;
      outStream << "Mesh_darcy_adv_diff: numSideSets = "     << meshSideSets_->size() << std::endl;
    }
    return meshSideSets_;
  }
  
}; // MeshManager_darcy_adv_diff
