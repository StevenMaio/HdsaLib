#ifndef PDEOPT_DYNAMIC_SHALLOW_ICE_BEDROCK_HPP
#define PDEOPT_DYNAMIC_SHALLOW_ICE_BEDROCK_HPP
...
#include "pde_shallow_ice_bedrock.hpp"

template <class Real>
class DynamicPDE_shallow_ice_bedrock : public DynamicPDE<Real> {
private:
  ...
  ROL::Ptr<PDE_shallow_ice_bedrock<Real> > pde_;
  Real theta_;
...
public:
  DynamicPDE_shallow_ice_bedrock(Teuchos::ParameterList &parlist) {
    pde_ = ROL::makePtr<PDE_shallow_ice_bedrock<Real>>(parlist);
  ...
  void Jacobian_zf(ROL::Ptr<Intrepid::FieldContainer<Real>> & jac,
                   ...
                   const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                   const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Jacobian_zf): Jacobian is zero.");
  }
  ...
  void Jacobian_zp(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & jac,
    ...
    pde_->setTime(told); 
    pde_->Jacobian_3(pde,uo_coeff,z_coeff,z_param);
    // EVALUATE NEW COMPONENT
    pde_->setTime(tnew);
    pde_->Jacobian_3(jac,un_coeff,z_coeff,z_param);
    for (int i = 0; i < size; ++i) {
      std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > J;
      fieldHelper_->splitFieldCoeff(J, jac[i]);
      std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > PDE;
      fieldHelper_->splitFieldCoeff(PDE, pde[i]);
      Intrepid::RealSpaceTools<Real>::scale(*(J[0]), theta_*dt);
      Intrepid::RealSpaceTools<Real>::scale(*(PDE[0]), (one-theta_)*dt);
      Intrepid::RealSpaceTools<Real>::add(*(J[0]), *(PDE[0]));
      // APPLY DIRICHLET CONDITIONS
	...
	for (int j = 0; j < numLocalSideIds; ++j) {
	  ...
	  for (int k = 0; k < numCellsSide; ++k) {
	    int cidx = bdryCellLocIds_[0][j][k];
	    for (int l = 0; l < numBdryDofs; ++l) {
		(*J[0])(cidx,fidx_[j][l]) = static_cast<Real>(0);
	    }
	  }
	}
      }
    Intrepid::RealSpaceTools<Real>::scale(*J[1], theta_*dt);
    Intrepid::RealSpaceTools<Real>::scale(*J[2], theta_*dt);
    // Combine the jacobians.
  ...
  void Hessian_uo_uo(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_uo_uo): Hessian is zero.");
  }
  ...
  void Hessian_uo_un(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_uo_un): Hessian is zero.");
  }
  ...
  void Hessian_uo_zf(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_uo_zf): Hessian is zero.");
  }
  ...
  void Hessian_uo_zp(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_uo_zp): Hessian is zero.");
  }
  ...
  void Hessian_un_uo(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_un_uo): Hessian is zero.");
  }
  ...
  void Hessian_un_un(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_un_un): Hessian is zero.");
  }
  ...
  void Hessian_un_zf(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_un_zf): Hessian is zero.");
  }
  ...
  void Hessian_un_zp(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_un_zp): Hessian is zero.");
  }
  ...
  void Hessian_zf_uo(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zf_uo): Hessian is zero.");
  }
  ...
  void Hessian_zf_un(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zf_un): Hessian is zero.");
  }
  ...
  void Hessian_zf_zf(ROL::Ptr<Intrepid::FieldContainer<Real>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zf_zf): Hessian is zero.");
  }
  ...
  void Hessian_zf_zp(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zf_zp): Hessian is zero.");
  }
  ...
  void Hessian_zp_uo(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zp_uo): Hessian is zero.");
  }
  ...
  void Hessian_zp_un(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zp_un): Hessian is zero.");
  }
  ...
  void Hessian_zp_zf(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zp_zf): Hessian is zero.");
  }
  ...
  void Hessian_zp_zp(std::vector<std::vector<ROL::Ptr<Intrepid::FieldContainer<Real>>>> & hess,
                     ...
                     const ROL::Ptr<const Intrepid::FieldContainer<Real>> & z_coeff = ROL::nullPtr,
                     const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    throw Exception::Zero(">>> (PDE_shallow_ice_bedrock::Hessian_zp_zp): Hessian is zero.");
  }
...
private:
 ...
 void computeDirichlet(void) {
  ...
  Real DirichletFunc(const std::vector<Real> & coords, int sideset, int locSideId) const {
    ...
    //std::cout << "coords[0] = " << coords[0] << " and coords[1] = " << coords[1] << " and locSideId = " << locSideId << " and val = " << val << std::endl;
    return val;
  }
...
public:
  ...
  {
    pde_->Update_Z_input(Z);
  }

}; // DynamicPDE_shallow_ice_bedrock
