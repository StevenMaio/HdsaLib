...
void Set_beta(HDSA::Ptr<Tpetra::MultiVector<> > & z_ptr, Teuchos::RCP<Teuchos::ParameterList> & parlist)
{
  ...
  RealT beta_scale = parlist->sublist("Problem").get("Sliding Scale", 1.0);
  ...
  for(int k = 0; k < num_coeff_load; k++)
    {
      z_ptr->replaceGlobalValue(3*k,0,beta_coeff[k] + std::log(beta_scale));
    }
}
...
int main(int argc, char *argv[]) {
  ...
  dyn_con->outputTpetraVector(z_ptr,"true_beta.txt");
