...
#include "dynpde_shallow_ice_bedrock.hpp"
...
private:
  ...
  HDSA::Ptr<DynamicPDE_shallow_ice_bedrock<RealT> > pde_shallow_ice_;
...
public:
  ...
  { 
    ...
    // Initialize PDE
    HDSA::Ptr<DynamicPDE<RealT> > pde = HDSA::makePtr<DynamicPDE_shallow_ice_bedrock<RealT> >(*parlist);
    pde_shallow_ice_ = HDSA::dynamicPtrCast<DynamicPDE_shallow_ice_bedrock<RealT> >(pde);  
    ...
    bool checkDeriv = true; // If set to true, uncomment Load_Optimal_Solution above, keep it commented when not running derivative check
    if ( checkDeriv ) 
      {
