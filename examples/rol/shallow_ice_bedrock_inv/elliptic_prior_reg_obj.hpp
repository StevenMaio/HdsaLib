...
public:
                ...
                const ROL::Ptr<const std::vector<Real>> &z_param = ROL::nullPtr)
  {
    ...
    Intrepid::RealSpaceTools<Real>::add(*R[1], *eye_term);
    Intrepid::RealSpaceTools<Real>::add(*R[2], *eye_term);
                  ...
                  const ROL::Ptr<const std::vector<Real>> &z_param = ROL::nullPtr)
  {
    ...
    Intrepid::RealSpaceTools<Real>::add(*J[1][1], *eye_term);
    Intrepid::RealSpaceTools<Real>::add(*J[2][2], *eye_term);
...
public:
                ...
                const ROL::Ptr<const std::vector<Real>> &z_param = ROL::nullPtr)
  {
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*R[0],
                                                  ...
                                                  *(fe_->NdetJ()),
                                                  Intrepid::COMP_CPP, false);
    ROL::Ptr<Intrepid::FieldContainer<Real>> U1_eval = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*R[1],
                                                  ...
                                                  *(fe_->NdetJ()),
                                                  Intrepid::COMP_CPP, false);
    ROL::Ptr<Intrepid::FieldContainer<Real>> U2_eval = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
...
template <class Real>
class Elliptic_Prior_Regularization_Objective_SimOpt : public ROL::Objective_SimOpt<Real>
{
...
public:
  Elliptic_Prior_Regularization_Objective_SimOpt(const ROL::Ptr<const Teuchos::Comm<int>> &comm, const ROL::Ptr<Teuchos::ParameterList> &parlist, ROL::Ptr<std::ostream> &outStream, bool construct_operators = false)
      : ROL::Objective_SimOpt<Real>()
  {
    ...
    ROL::Ptr<Linear_PDE_Constraint<Real>> pdecon = ROL::dynamicPtrCast<Linear_PDE_Constraint<Real>>(con_);
    ROL::Ptr<Assembler<Real>> assembler = pdecon->getAssembler();
    ROL::Ptr<Tpetra::MultiVector<>> prior_mean_ptr = assembler->createControlVector();
    ...
    {
      ROL::Ptr<ROL::Vector<Real>> zp = prior_mean_->clone();
      ROL::Ptr<ROL::Vector<Real>> Kz = zp->clone();
      ROL::Ptr<ROL::Vector<Real>> Mz = zp->clone();
      ROL::Ptr<ROL::Vector<Real>> tmp1 = zp->clone();
      int n = zp->dimension();
      ...
      fout.close();
      std::cout << "I finished writing the matrices" << std::endl;
    }
  }
...
}; // Elliptic_Prior_Regularization_Objective_SimOpt
...
public:
  ...
  Real value(const ROL::Vector<Real> &z, Real &tol) override
  {
    ...
    tmp->axpy(-1.0, *prior_mean_);
    ROL::Ptr<ROL::Vector<Real>> Az = z.clone();
    con_->applyJacobian_1(*Az, *tmp, z, z, tol);
    ROL::Ptr<ROL::Vector<Real>> MinvAz = z.clone();
