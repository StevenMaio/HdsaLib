...
int main(int argc, char *argv[])
{
  ...
  pde_shallow_ice->setParameter(*theta_std.get_std_vec());
  ...
  weights_misfit[0] = parlist->sublist("Problem").get("State Cost", 1.0) * (static_cast<RealT>(nt) / T);
  ...
  // Regularization
  HDSA::Ptr<ROL::Objective<RealT>> robj_reg = HDSA::makePtr<Elliptic_Prior_Regularization_Objective<RealT>>(comm->Get_Teuchos_Communicator(), parlist, outStream, construct_matrices);

  std::vector<RealT> rs_weights = std::vector<RealT>(2);
  rs_weights[0] = 1.0;
  rs_weights[1] = 1.0;
  ...
  obj_vec[1] = robj_reg;
  HDSA::Ptr<ROL::Objective<RealT>> robj = HDSA::makePtr<RS_Objective<RealT>>(rs_weights, obj_vec);
