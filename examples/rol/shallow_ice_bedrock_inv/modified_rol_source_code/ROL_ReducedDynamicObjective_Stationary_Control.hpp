#ifndef ROL_REDUCEDDYNAMICOBJECTIVE_STATIONARY_CONTROL_HPP
#define ROL_REDUCEDDYNAMICOBJECTIVE_STATIONARY_CONTROL_HPP
...
class ReducedDynamicObjective_Stationary_Control : public Objective<Real> {
...
public:
  ReducedDynamicObjective_Stationary_Control(const Ptr<DynamicObjective<Real>>  &obj,
  ...
  Real value( const Vector<Real> &x, Real &tol ) {
    if (!isValueComputed_) {
      ...
      for (size_type k = 1; k < Nt_; ++k) {
        ...
        con_->update(*uhist_[index-1], *uhist_[index], x, timeStamp_[k-1]);
        obj_->update(*uhist_[index-1], *uhist_[index], x, timeStamp_[k-1]);
        // Solve state on current time interval
        con_->solve(*cprimal_, *uhist_[index-1], *uhist_[index], x, timeStamp_[k-1]);
        // Compute objective function value on current time interval
        valk  = obj_->value(*uhist_[index-1], *uhist_[index], x, timeStamp_[k-1]);
  ...
  void gradient( Vector<Real> &g, const Vector<Real> &x, Real &tol ) {
    g.zero();
    ROL::Ptr<Vector<Real> > gc = g.clone();
    const Real one(1);
    ...
    // Update dynamic constraint and objective
    con_->update(*uhist_[uindex-1], *uhist_[uindex], x , timeStamp_[Nt_-2]);
    obj_->update(*uhist_[uindex-1], *uhist_[uindex], x , timeStamp_[Nt_-2]);
    ...
    updateGradient(*gc,    *lhist_[lindex],
                   *uhist_[uindex-1], *uhist_[uindex],
                   x,    timeStamp_[Nt_-2]);
    g.set(*gc);
    ...
    for (size_type k = Nt_-2; k > 0; --k) {
      ...
      // Update dynamic constraint and objective
      con_->update(*uhist_[uindex-1], *uhist_[uindex], x, timeStamp_[k-1]);
      obj_->update(*uhist_[uindex-1], *uhist_[uindex], x, timeStamp_[k-1]);
  ...
  void hessVec( Vector<Real> &hv, const Vector<Real> &v, const Vector<Real> &x, Real &tol ) {
    if (useHessian_) {      // Must first solve the state and adjoint equations
      solveState(x);
      solveAdjoint(x);
      // Now compute Hessian
      const Real one(1);
      hv.zero(); // zero for the nonexistant zeroth control interval
      ROL::Ptr<Vector<Real> > hvc = hv.clone();
      ...
      for (size_type k = 1; k < Nt_; ++k) {
        ...
        if (!useSymHess_) {
          ...
          addMixedHessLag(*hvc,       *lhist_[lindex],
	...
	hv.axpy(1.0,*hvc);
      ...
      if (useSymHess_) {
        // Add mixed derivative Hessian
        addMixedHessLag(hv,   *lhist_[lindex],
                        *whist_[uindex-1], *whist_[uindex],
                        *uhist_[uindex-1], *uhist_[uindex],
                        x,    timeStamp_[Nt_-2]);
      }
      // Add adjoint sensitivity to Hessian
      addAdjointSens(hv,   *phist_[lindex],
                     *uhist_[uindex-1], *uhist_[uindex],
                     x,    timeStamp_[Nt_-2]);
      // Run reverse time stepper
      for (size_type k = Nt_-2; k > 0; --k) {
  ...
  Real updateSketch(const Vector<Real> &x, const Real tol) {
    ...
    while (flag) {
      ...
      for (size_type k = 1; k < Nt_; ++k) {
        ...
        con_->value(*cprimal_, *uhist_[0], *uhist_[1], x, timeStamp_[k-1]);
        cnorm = cprimal_->norm();
...
#endif // ROL_REDUCEDDYNAMICOBJECTIVE_STATIONARY_CONTROL_HPP
