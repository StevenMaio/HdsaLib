#ifndef PDE_BRANDES_GRIESSE_PARAMETER_HPP
#define PDE_BRANDES_GRIESSE_PARAMETER_HPP
...
template <class Real>
class PDE_Brandes_Griesse_Parameter : public PDE<Real> {
private:
  ...
  std::vector<std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > > Generate_Boundary_Alpha_Values(const std::vector<Real> & z_param)
  {
    ...
    for (int i=0; i<numSidesets; ++i) {
      ...
      for (int j=0; j<numLocSides; ++j) {
	    ...
	    (*bdryCellAlphaValues[i][j])(k, l) = AlphaFunc(dofpoint,z_param);
          }
        }
      }
    }
...
public:
  PDE_Brandes_Griesse_Parameter(Teuchos::ParameterList &parlist) {
  ...
  void residual(ROL::Ptr<Intrepid::FieldContainer<Real> > & res,
    ...
    std::vector<std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > > bdryCellAlphaValues = Generate_Boundary_Alpha_Values(*z_param);
    ...
    fe_vol_->evaluateValue(valZ_eval, Z_input);
  ...
  void Jacobian_3(std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > & jac,
    ...
    for(int ii = 0; ii < size; ++ii) 
      {
	jac[ii] = ROL::makePtr<Intrepid::FieldContainer<Real> >(c, f);

	std::vector<Real> param = std::vector<Real>(size,0.0);
	param[ii] = 1.0;
	std::vector<std::vector<ROL::Ptr<Intrepid::FieldContainer<Real> > > > bdryCellAlphaValues = Generate_Boundary_Alpha_Values(param);
		...
		for (int k = 0; k < numCellsSide; ++k) 
		  {
		    ...
		    for (int l = 0; l < numBdryDofs; ++l) 
		      {
			(*jac[ii])(cidx,fidx_[j][l]) += (*robinRes)(k,fidx_[j][l]);
		      }
		  }
	      }
	    }
